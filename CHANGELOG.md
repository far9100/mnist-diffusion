...
- `2026-07-23-04` test — 完整版 fix_tasks T9 第二架構真跑＋§5.4.3：`results/tstr_protocol_ablation_wrn.json`
  （WRN-16-4、seed10 三 cell、epochs {15,50}、reps 3；GPU 競用下較慢）。WRN 重跑得與 ResNet-18 **完全相同**
  的排序（15ep [w1,w1.5,w2.5]、50ep [w1.5,w1,w2.5]、w2.5 恆最差、rank_stable=False），逐 cell 值亦相近
  （w1 15/50ep 59.25/61.01、w1.5 58.19/61.22、w2.5 51.25/55.69）。故 w1↔w1.5 epoch 翻轉為**架構穩健**、非
  ResNet-18 特有，坐實「噪聲地板為協定內生」。§5.4.3 補一句；verifier 增 2 條，重跑 OK 387／MISMATCH 0；凍結
  未動。fix_tasks Stage 4 全部（含完整版 T8／T5b 與 T9 第二架構）完成。
- `2026-07-23-05` test — 任務書 fix_tasks T5b（A8）MNIST 側 w<1 scout 真跑＋§6.3：新增
  `src/experiments/run_mnist_subunity_scout.py`、`results/mnist_subunity_scout.json`。MNIST（seeds 0/1/2、
  per_digit 1000、tstr_epochs 20）TSTR 隨 w 單調上升至 g1——g0.5（92.63）< g0.75（95.21）< g1（97.30，峰），
  三 seed 皆單調、每個 w<1 皆低於同 seed 之 g1；w<1 因往無條件內插致 precision/coverage 降（g0.5
  0.605/0.431）。故 MNIST 之 oracle g1 與 CIFAR-100 之 w1 同為真實內部峰、非網格下界假影（回應正文「w1 恰在
  網格下界」保留）。共用 T1a/T1b 之 `results/mnist_gen_cache/` 同批影像、g1 取凍結 selector_signal 不重算。
  §6.3 subunity 段補 MNIST；verifier 增 3 條，重跑 OK 390／MISMATCH 0；凍結未動、兩資料集 c6 duel ALL_MATCH。
  T5b 全資料集（CIFAR-100 3-seed＋MNIST）完成。
- `2026-07-23-06` add — 任務書 fix_tasks T6a（A9）重現包工具與 README 重現指南：新增
  `tools/make_release_bundle.py`（對發布用權重 cifar10/100_cfg.pt、cifar10/100_judge.pt、ddpm_mnist.pt、
  mnist_cnn.pt＋選配 inception 逐檔算 sha256，盤點 p1_assets／p1_assets_cifar100 成清單，產
  `results/release_bundle_manifest.json` 與上傳說明 `docs/release_bundle.md`；權重合計 1.6GiB、P1 資產
  3.6GiB，不上傳、由作者執行 Zenodo/HF）；README 增「重現指南（從權重到每張表）」節——取得權重＋sha256
  核對、對帳指令、從 confirmatory 到各表的 driver 指令序列（以各 JSON metadata.argv 為準）。純工具＋文字、
  無 GPU；T6a 三子項（.gitignore 於 2026-07-21-04 已做、本次補重現包工具與 README）全部完成。
