...
可量測、仍複製。護城河對決另顯示：一個 coverage-boosting 的 guidance 方法（Chamfer）產生的合成資料，
...
機制的**介入**證據（預先登記 D3 補充，`results/cifar100_d3_intervention.json`）：C3 coverage-matched
pruning。把 w2.5（coverage 0.700、TSTR 50.66）移除 13606 個離真實流形最近之樣本、剪至 w1 之 coverage
水準（0.481），TSTR 降至 45.75；等計數隨機剪枝降至 45.86。兩者差 −0.11pp，遠在 σ_cls（2.96pp）內、
且 N_retrain=2（underpowered）。coverage-matched 剪枝之 TSTR 代價與隨機剪枝**無法區分**——TSTR 掉幅
由樣本數減少解釋，非由移除 coverage-carrying 樣本。介入臂**未對 coverage 之因果角色提供支持**，與
CIFAR-10 C2/C3/C5 同向（[CHANGELOG 2026-07-09-08](../CHANGELOG.md#2026-07-09)）。
...
（rep 數不對稱）、單 seed（10）——屬 exploratory 護城河對照，非窮盡 benchmark。

## 5. 誠實負面與貢獻定位

CaF 作為「普適免訓練選擇器」的原始賣點，在 CIFAR-10 敗於更便宜的 FID-min、在 CIFAR-100 與其打平；
且其「選 vanilla、不修改」的操作點在護城河對決中敗於較複雜的 Chamfer（§4.5，即使 Chamfer 用任務
