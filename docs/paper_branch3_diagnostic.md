...
## 4.5 matched-budget 對照（H3）：Chamfer 勝 vanilla；coverage 反低為量測解析度相依
...
- **coverage proxy 讀數偏低，但此讀數存疑**：兩個 Chamfer 臂的 DINOv2 coverage 皆低（0.44–0.48，低於
  ...
  但雙向公平化重跑（thesis §5.6.1）證此讀數非單向簡化假影（雙向與單向皆低@224 約 0.46）、為量測解析度相依
  （同影像於 112 導引空間 coverage 反高約 0.85）。

意涵：CaF 的「選 vanilla、不修改」在效用上敗於較複雜的 Chamfer（即使 Chamfer 用任務無關特徵）。至於「沒有便宜代理能事前把 Chamfer 的高效用集排在前面」一點，因本文
Chamfer 的 coverage 反低為量測解析度相依（§5.6.1：224 低、112 導引空間高；雙向重測仍低），暫不據以推廣。這在
效用面否定 CaF 的操作優勢、強化
