...
沒有單一便宜代理能跨資料集普遍可靠。同一套 coverage 選擇器在 MNIST 選中效用最優組態（regret 0）；在
CIFAR-10 上，它敗給更便宜的 FID-min baseline（regret 3.69 對 0.91）；在 CIFAR-100 上與 FID-min 打平
（兩者均為 0.76）。反之，FID-min 在兩個 CIFAR 尺度近乎最優，卻在 MNIST 上選錯組態。本文給出此不可靠
性的三個診斷來源——which-FID 兩表徵不分離、選擇器的 Pareto 失明引理（當
網格中存在同時支配所有 oracle 的組態時，單調選擇器結構性地選不到 oracle）、變異分解的功效意涵——並
報告 coverage 與 near-boundary 的雙段機制在最難的 CIFAR-100 仍可量測、仍複製
（但介入證據顯示其因果角色未獲支持）。與 Chamfer 的 matched-budget 對照另顯示：一個 coverage-boosting 的 guidance 方法
（Chamfer）產生的合成資料，下游效用高於任何 vanilla CFG 組態，但其 coverage 反而低——便宜代理無法
事前辨識此高效用集，再證不可靠。貢獻由「一個普適的免訓練選擇器」誠實地降為「代理可靠性之資料集
相依性及其量測方法學」。
...
which-FID non-separation across representation spaces, a Pareto-blindness lemma for the selector,
...
a coverage-boosting guidance method (Chamfer) yields higher downstream utility than any vanilla
CFG configuration while exhibiting lower coverage — a cheap proxy cannot identify this
high-utility set a priori. The contribution is honestly demoted from "a universal training-free
...
1. **經驗發現（主結果）**：哪個便宜代理可靠，隨資料集反轉。同一套 coverage 選擇器在近可分的 MNIST
   選中效用最優組態（regret 0），在較難的 CIFAR-10 與 CIFAR-100 卻敗給或打平更便宜的 FID-min
   baseline；而 FID-min 在 CIFAR 近最優、在 MNIST 卻會選錯。代理的可靠性本身是資料集相依的。
...
依序報告 MNIST、CIFAR-10、CIFAR-100 的結果、三個診斷來源、機制觀察與介入，以及護城河對決。第六章
討論主結果（代理可靠性之資料集相依性）、誠實負面、限制與相關工作差異化。第七章結論並列出未來工作。
...
- **guidance 強度 w（CFG）**（MNIST sandbox 早期記為 g，本文統一以 w 表示）：classifier-free guidance
  以一個無條件與一個有條件預測的外插，控制取樣往指定類別集中的程度。提高 w 會把取樣分佈往該類別的
  prototype 集中並銳化（先去噪再銳化，denoise-then-sharpen；Bradley & Nakkiran, 2024）。銳化提升單張
  影像看起來的「典型性」與保真度，但同時抽走靠近決策邊界、低 margin 的非典型樣本。
...
**路線三：品質度量與下游效用的關係**。「保真度（FID）未必等於下游效用」的觀察並非本文首創：差分隱私
擴散文獻已順帶觀察過取樣隨機性（η）與下游效用的關係——Dockhorn et al.（TMLR 2023）§5.2 指出隨機取樣
對 FID 關鍵、對下游準確率影響較小，是「FID 與下游效用不同向」的既有旁證；上述 guidance 方法亦隱含
此點。本文不宣稱此觀察為新發現，而聚焦其**可操作的後果**：能否用便宜代理事前選中效用最優組態。
...
功效意涵。特別地，本文的護城河對決（第 5.6 節）顯示，即使是 coverage-boosting 的 guidance 方法
（Chamfer）所產生的高效用合成集，也無法被便宜代理（FID、coverage）事前排在前面——這對「用便宜代理
選組態」的整個路線構成一個誠實的反例。
...
FD-DINOv2 與 clean-fid 兩個 FID 空間；FID-min baseline 以 matched-probe（與選擇器同一份真實 probe）
計算。每個量測 driver 的輸出 metadata 強制記錄可重現所需的全部參數（nearest_k、有效 k=min(k, n−1)、
...
取梯度、以 −chamfer_weight×grad 為位移併回 DDIM 更新。護城河對決設定：chamfer_weight＝1.0、每類 16 個
...
同向；而 precision 在 w3 達峰（0.966）、TSTR 卻在 w1 達峰。故一個 fidelity/precision 驅動的代理會選到
w3 一帶而選錯，coverage 驅動的 CaF 則三個 seed 全選中 oracle w1，regret 0.00±0.00、rank 1/6、top-3
命中 100%。此為「coverage 可靠、fidelity 不可靠」的乾淨案例。限制：MNIST 近可分，真實資料 near-boundary
...
跨零、可交換性受 gseed 碰撞影響，穩健性須由 CIFAR-100 獨立複製回答。顯著亦不寫「coverage 驅動效用」
的因果句。
...
- **C1（which-FID）不分離**：Inception clean-fid 的 argmin=w1.5、TSTR 的 argmax=w1，相隔 1 格；逐
  seed 分離格步 >1 的數目 0/8。判不分離。口徑範圍限制：CIFAR-100 未算 per-config FD-DINOv2（無對應
  P1 跑），which-FID 只在 Inception 空間評估。
...
|---|---|---|---|---|---|
| MNIST | w1 | w1 | 0.00 | 不可靠（選 w3 一帶） | coverage（CaF 選中 oracle） |
| CIFAR-10 | 依 seed | w2.5 | 3.69 | 0.91（2 勝 1 負） | FID-min |
| CIFAR-100 | w1 | w1.5 | 0.76 | 0.76（打平） | FID-min（更便宜） |

（MNIST：`selector_signal.json`；CIFAR-10：`cifar10_c6_fidmin_duel.json`；CIFAR-100：
`cifar100_c6_fidmin_duel.json`。）

判讀——哪個便宜代理可靠，隨資料集反轉：coverage 選擇器在近可分的 MNIST 可靠、在較難的 CIFAR
失效；FID-min 在 CIFAR 近最優、在 MNIST 會選錯。代理的可靠性本身是資料集相依的。

![圖 5.2](figures/fig_selector_reversal.png)

**圖 5.2**　三尺度選擇器 regret 對照（越低越好）。coverage 選擇器（CaF／CaF-v2）在 MNIST regret 0、
在 CIFAR-10 敗於更便宜的 FID-min、在 CIFAR-100 與其打平；保真代理則相反。可靠的便宜代理隨資料集
反轉。（MNIST 無 FID-min 臂，保真代理以 precision-argmax 的衍生 regret 表示。）

## 5.4 診斷來源：為何沒有便宜代理普遍可靠

### 5.4.1 which-FID 兩表徵不分離（C1）

若「保真最優偏離效用最優」成立，換一個 FID 的特徵空間或許能救。資料顯示不能：CIFAR-10 上 FD-DINOv2
與 Inception 兩空間的 FID-argmin 都與 TSTR-argmax 相鄰或重合（格步 1 與 0），依凍結口徑皆不分離；
CIFAR-100 上（僅 Inception 空間可得）逐 seed 0/8 不分離。即 FID 的 argmin 與 TSTR 的 argmax 落在相鄰
或同一組態，換表徵空間也不拉開，保真代理與效用最優並未分離到能靠「挑另一種 FID」可靠選中的程度。
...
**介入證據**（`results/cifar100_d3_intervention.json`）：C3 coverage-matched pruning。把 w2.5
（coverage 0.700、TSTR 50.66）移除 13606 個離真實流形最近之樣本、剪至 w1 之 coverage 水準（0.481），
TSTR 降至 45.75；等計數隨機剪枝降至 45.86。兩者相差 −0.11pp，遠小於分類器訓練標準差 σ_cls＝2.96pp；
且重訓次數僅 N=2，統計功效不足（underpowered）。coverage-matched 剪枝之 TSTR 代價與隨機剪枝無法區分
——TSTR 掉幅由樣本數減少
解釋，非由移除 coverage-carrying 樣本。介入臂未對 coverage 之因果角色提供支持，與 CIFAR-10 之
偏相關裁決（C2）、剪枝介入（C3）等機制分析同向。（重生成之合成集對 confirmatory seed-10 逐位重現，`results/cifar100_regen_reconcile.json`
rel_delta 全 0。）
...
## 5.6 與 Chamfer 之 matched-budget 對照（H3，下稱護城河對決）：Chamfer 勝 vanilla，但增益不見於 coverage
...
- **但增益不見於 coverage proxy**：兩個 Chamfer 臂的 DINOv2 coverage 皆低（0.44–0.48，低於 vanilla
  w1.5 的 0.643、接近最低的 w1 0.481），卻有最高 TSTR。Chamfer 的高效用集會被 coverage 排在後段——
  又一個便宜代理不追蹤效用的例子。

意涵：CaF 的「選 vanilla、不修改」在效用上敗於較複雜的 Chamfer（即使 Chamfer 用任務無關特徵），且
沒有便宜代理（FID、coverage）能事前把 Chamfer 的高效用集排在前面。限制：chamfer_weight 固定 1.0、
每類 16 exemplar、Chamfer 臂 N=3 而 vanilla 臂沿用 confirmatory 之 5 rep 值（rep 數不對稱）、單 seed
（10）——屬 exploratory 護城河對照，非窮盡 benchmark。

![圖 5.6](figures/fig_h3_duel.png)

**圖 5.6**　H3 三臂 matched-budget 對決（seed 10）。Chamfer 兩變體（任務無關 DINOv2、任務對齊 judge）
的 TSTR 皆高於 vanilla w1.5，但其 DINOv2 coverage 反而較低（0.44–0.48 對 0.643）。高效用集會被 coverage
代理排在後段——便宜代理無法事前辨識。（`results/cifar100_h3_duel_dinov2.json`、`_judge.json`）

---

# 第六章 討論

## 6.1 主結果：代理可靠性之資料集相依性

彙整第五章：哪個便宜代理可靠，隨資料集反轉。coverage 選擇器在近可分的 MNIST 可靠（CaF 選中 oracle、
regret 0），在較難的 CIFAR-10／100 失效（敗或平更便宜的 FID-min）；FID-min 在 CIFAR 近最優、在 MNIST
會選錯。故「用哪個便宜代理」沒有普適答案——**代理的可靠性本身是資料集相依的**。這是一個否定但可
操作的結論：它告訴實務者，不能把在一個尺度上有效的便宜代理，無條件搬到另一個尺度。
...
**宣稱範圍：兩型、非連續譜**。須誠實界定此「資料集相依性」的證據強度。三個資料集中，CIFAR-10 與
CIFAR-100 行為相同（FID-min 近最優、CaF 敗或平），真正相異的只有 MNIST。因此本文的反轉實質是
**近可分（MNIST 型）對非可分（CIFAR 型）的單一型別對比**，而非沿資料集難度連續變化的譜。本文據此把
宣稱收斂為「在近可分與非可分之間，可靠的便宜代理發生反轉」，不宣稱它隨任意資料集平滑改變；要把它
提升為連續譜，需要更多落在兩型之間的資料點。

## 6.2 誠實負面與貢獻定位

CaF 作為「普適免訓練選擇器」的原始賣點，在 CIFAR-10 敗於更便宜的 FID-min、在 CIFAR-100 與其打平；
且其「選 vanilla、不修改」的操作點在護城河對決中敗於較複雜的 Chamfer（即使 Chamfer 用任務無關
特徵）。此賣點不成立。我們據實把貢獻降為診斷：代理可靠性之資料集相依性、其量測方法學（PRDC、
FD-DINOv2、matched-probe FID-min 對決、變異分解、Pareto 失明引理），以及一個誠實記錄——便宜的
FID-min baseline 在 CIFAR 尺度常已近最優，任何更貴的選擇器須先勝過它。

## 6.3 限制

- **因果未證，且「未證」不等於「被否證」**：coverage 承載效用之因果宣稱在兩資料集皆未經介入證實；
  CIFAR-100 介入臂 N_retrain=2、差值落在 σ_cls 內，統計功效不足（underpowered）。須強調這意味著
  「無法判定」而非「因果被否證」——現有功效不足以區分「無效應」與「有效應但被雜訊淹沒」。機制目前為
  觀察性複製。
...
- **CIFAR-100 未算 per-config FD-DINOv2**：which-FID 在 CIFAR-100 只在 Inception 一個空間評估，與
  CIFAR-10 的雙空間不對稱。
- **coverage 絕對值跨資料集不可比**：CIFAR-100 的 per-class 樣本數（500）與 CIFAR-10（1000）不同，
  此限制於揭盲前登記，並陳兩資料集 coverage 曲線時須用資料集內正規化或明記不可比。
- **H3 為 exploratory**：單 seed、N=3、chamfer_weight 固定 1.0、rep 數不對稱，非窮盡 benchmark。
...
須誠實面對護城河對決對本文差異化的削弱。§5.6 顯示修改 sampler 的 Chamfer 勝過 CaF 的操作點，且無便宜
代理能事前挑出 Chamfer 的高效用集——這削弱了原先主打的「操作點差異化（免訓練、不修改、可組合）」作為
賣點。在此結果下，本文剩下的貢獻明確為三項、且都不依賴 CaF 作為一個可用的選擇器：(1) **診斷方法學**
——預先登記與凍結四要件、變異分解與功效分析、which-FID 交叉裁決、matched-probe FID-min 對決；
(2) **Pareto 失明引理**——對「單調選擇器在支配情形下必然失明」的理論刻畫；(3) **一條路線的邊界（負面
知識）**——「用便宜代理選組態」在非可分資料上不可靠，甚至連 coverage-boosting guidance 產生的高效用集
都排不到前面。換言之，貢獻是方法與邊界，而非一個更好的選擇器。
...
的代理選中 TSTR 最優的取樣組態」，得到否定但可操作的結論——沒有單一便宜代理能跨資料集普遍可靠，
代理可靠性本身是資料集相依的。我們給出此不可靠性的三個診斷來源（which-FID 不分離、Pareto 失明引理、
變異分解的功效意涵），報告雙段機制在最難的 CIFAR-100 仍可量測、仍複製但因果未證，並以護城河對決
顯示便宜代理連 coverage-boosting guidance 的高效用集都排不到前面。貢獻由「普適免訓練選擇器」誠實
降為「代理可靠性之資料集相依性及其量測方法學」，並附一個可重用的預先登記量測堆疊與一條 Pareto
失明引理。

**未來工作**：(1) 為 CIFAR-100 補算 per-config FD-DINOv2，使 which-FID 的雙表徵評估在兩尺度對稱；
(2) 提高機制介入的重訓次數（脫離 underpowered），把 coverage 的因果角色從「未證」推進到有功效的
否證或支持；(3) 把 H3 護城河對決擴為多 seed 與 chamfer_weight 掃描，使其可作結論；(4) 於更高解析度
或 ImageNet 子集複製，檢驗結論是否 scale。

---

# 參考文獻

（出處已查證並附 arXiv 編號；系上格式（如 IEEE／APA）之標點與頁碼於排版階段微調。）
...
5. Bradley, A., & Nakkiran, P. (2024). Classifier-Free Guidance is a Predictor-Corrector.
   *NeurIPS 2024 M3L Workshop*. arXiv:2408.09000.（denoise-then-sharpen 觀點）
...
15. Dockhorn, T., Cao, T., Vahdat, A., & Kreis, K. (2023). Differentially Private Diffusion Models.
    *Transactions on Machine Learning Research (TMLR)*. arXiv:2210.09929.（§5.2 ablation：隨機取樣
    對 FID 關鍵、對下游準確率影響較小——FID 與下游效用不同向的既有順帶觀察）
...
| D3 介入臂 cov-matched≈random | `cifar100_d3_intervention.json`、`cifar100_regen_reconcile.json`（rel 全 0） | 已跑 |
| H3 護城河對決 Chamfer 勝 vanilla、coverage 低 | `cifar100_h3_duel_dinov2.json`、`_judge.json` | 已跑 |
...
依計畫 `C:\Users\fartw\.claude\plans\cozy-crunching-curry.md` 分七階段撰寫：
...
Q5 P1 加值實驗（CIFAR-100 per-config FD-DINOv2、更高功效機制介入、H3 多 seed／weight）須作者授權後
另行啟動；P2（更高解析度／ImageNet）列未來工作。
