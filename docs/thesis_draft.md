...
（Chamfer）產生的合成資料，下游效用高於任何 vanilla CFG 組態（其 coverage 讀數在本文的單向簡化
Chamfer、單 seed 下反低，惟官方雙向實作方向相反〔coverage 上升〕，此讀數可能為簡化假影）。貢獻由「一個普適的免訓練選擇器」誠實地降為「coverage 型代理可靠性之
...
任何 vanilla 組態；至於「其 coverage 反低、便宜代理排不到前面」的讀數，因本文 Chamfer 為單向簡化、單 seed
而存疑（官方雙向實作方向相反），列為 exploratory 觀察、待雙向公平化後方可定論。
...
## 5.6 與 Chamfer 之 matched-budget 對照（H3）：Chamfer 勝 vanilla；coverage 讀數偏低但可能為簡化假影
...
- **coverage proxy 讀數偏低，但此讀數在本文設定下存疑**：兩個 Chamfer 臂的 DINOv2 coverage 皆低
  （0.44–0.48，低於 vanilla w1.5 的 0.643、接近最低的 w1 0.481），卻有最高 TSTR，表面上又是一個便宜
  代理不追蹤效用的例子。但本文的 Chamfer 為**單向簡化重寫**（導引僅取單向 coverage 項，見方法章）、
  且為單 seed；官方雙向實作報告的卻是 coverage **上升**（0.603→0.912），方向相反。故「增益不見於
  coverage」在本文單向、單 seed 設定下成立，但很可能是簡化假影、不應據以推廣；欲做實需雙向 Chamfer、
  多 seed、且導引頻率與官方對齊之公平化重跑。

意涵：CaF 的「選 vanilla、不修改」在效用上敗於較複雜的 Chamfer（即使 Chamfer 用任務無關特徵）。至於
「沒有便宜代理（FID、coverage）能事前把 Chamfer 的高效用集排在前面」一句，須附前段保留——coverage
排序偏低很可能源於本文的單向簡化，官方雙向實作方向相反，故此點待雙向公平化後方可定論。限制：Chamfer
為單向簡化重寫、chamfer_weight 固定 1.0、每類 16 exemplar、Chamfer 臂 N=3 而 vanilla 臂沿用 confirmatory
...
的 TSTR 皆高於 vanilla w1.5，但其 DINOv2 coverage 反而較低（0.44–0.48 對 0.643）。此 coverage 偏低讀數
出自本文的單向簡化 Chamfer、單 seed，然官方雙向實作報告 coverage 上升（0.603→0.912），故可能為簡化
假影、須雙向公平化後方可定論。（`results/cifar100_h3_duel_dinov2.json`、`_judge.json`）
...
- **H3 為 exploratory，且 coverage 讀數可能為簡化假影**：單 seed、N=3、chamfer_weight 固定 1.0、rep 數
  不對稱，非窮盡 benchmark；且本文 Chamfer 為單向簡化重寫，其「coverage 反低」與官方雙向實作（報告
  coverage 上升 0.603→0.912）方向相反，該讀數須雙向公平化後方可定論。
...
的高效用集都排不到前面」一點，因本文 Chamfer 為單向簡化、其 coverage 讀數可能為簡化假影，須雙向公平化後
方可定論）。在此結果下，本文剩下的貢獻明確為三項、且都不依賴 CaF 作為一個可用的選擇器：(1) **診斷方法學**
...
顯示 Chamfer 在下游效用上勝過任何 vanilla 組態（其 coverage 反低之讀數因本文單向簡化而存疑、待雙向公平化）。貢獻由「普適免訓練選擇器」誠實
