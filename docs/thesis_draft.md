...
代理可靠性本身是資料集相依的。我們給出此不可靠性的三個診斷來源（which-FID 交叉裁決、Pareto 失明引理、
變異分解的功效意涵；其中 which-FID 於 CIFAR-100 顯示 Inception 與 DINOv2 兩空間相反、保真代理可靠性
表徵相依），報告雙段機制在最難的 CIFAR-100 仍可量測、仍複製但因果未證，並以護城河對決
...
與其他剪枝／過濾型式的複製）；(3) 把 H3 護城河對決擴為多 seed 與 chamfer_weight 掃描，使其可作結論；
