...
- **epochs 消融（seed10）**：cell 排序**隨 epochs 翻轉**——15 epochs 下 w1（59.96）> w1.5（58.66），50 epochs
  ...
  內生」之直接證據：TSTR 的 15-epoch 訓練不足既放大 σ_cls、又使精細排序不穩。此翻轉為**架構穩健**：第二
  架構 WideResNet-16-4（2.77M 參數、與 ResNet-18 之 11.2M 迥異，`results/tstr_protocol_ablation_wrn.json`）重跑
  得**完全相同**的排序（15ep [w1, w1.5, w2.5]、50ep [w1.5, w1, w2.5]、w2.5 恆最差），故 w1↔w1.5 之 epoch 翻轉
  非 ResNet-18 特有、確為協定內生。
...
- **w1 為 TSTR 內部峰、非網格邊界假影**：以 off-protocol scout 將 CIFAR-100 seed10 網格延伸至 w<1
  ...
  條件性下降（precision 0.66、coverage 0.23）而 TSTR 掉。**三 seed（10/11/12）皆單峰、峰皆在 w1**（w0.5 42.07–42.74 < w0.75 55.80–56.36 < w1
  59.44–59.98 > w1.5 58.65–59.52），排序穩健。MNIST sandbox 亦同（`results/mnist_subunity_scout.json`，
  seeds 0/1/2、tstr_epochs 20）：TSTR 隨 w 單調上升至 g1——g0.5（92.63）< g0.75（95.21）< g1（97.30，峰），
  三 seed 皆單調、每個 w<1 皆低於同 seed 之 g1；w<1 同樣因往無條件內插而類別條件性下降（g0.5 precision
  0.605、coverage 0.431）。故兩個自訓資料集的 oracle 網格下界（CIFAR-100 w1、MNIST g1）皆為真實內部峰、
  非邊界假影。exploratory、off_protocol。
