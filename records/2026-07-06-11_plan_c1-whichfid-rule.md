...
### 3. 活結果空間（裁決二元、不分離內分兩落點，DINOv2 揭盲前鎖）

裁決層二元（分離／不分離），但「不分離」有兩個經驗落點，三落點揭盲前全預寫、零自由度：

- **DINOv2 ≥2 格步** → 分離 → 表徵依賴弱版本，標「非預期、削弱普適性」。
- **DINOv2 =1 格步** → 不分離（反證確立），記 1 格偏移為誠實邊註、與 Inception 0 格步對照（比照 pre-close 對 seed10 之邊註）。「一格偏移」≠「全重合」，邊註不改判、只誠實記經驗差異。
- **DINOv2 =0 格步** → 不分離（反證確立），兩空間全重合。
...
- **反證版（DINOv2 不分離）**：FD-DINOv2 之 argmin 與 TSTR-argmax 差 ≤1 格步，即 DINOv2 空間中 FID 亦不誤導。
  ...
  **落點區分（揭盲前預寫，零自由度）**：0 格步＝兩空間全重合；1 格步＝DINOv2 一格偏移、仍判不分離，記該 1 格
  偏移為誠實邊註、與 Inception 0 格步對照（比照 pre-close 對 seed10 之邊註）。二者皆反證確立，邊註不改判、
  只誠實記「一格偏移 ≠ 全重合」之經驗差異。
