...
- **介入臂（預註冊，D3 增列 `records/2026-07-08-02` §5.2.3）**：於 CIFAR-100 confirmatory 已生成合成集上跑一個
  ...
  剪枝條件數〔作者終簽〕；成本：無新取樣，僅剪枝＋重訓。**表徵口徑**：每觀察量標量測空間，DINOv2 主、
  Inception robustness（依 `records/2026-07-05-08` §4 型式）。
...
- **功效配置〔作者終簽〕**：建議 n_seed×n_rep 使 MDE ≤ 約 2.5pp（如 8×5 或 10×3，參 D4 決策單 MDE 表）；
  最終數字作者定。
...
- matched-probe 尺寸〔作者終簽〕：建議與 CaF probe 同尺寸（confirmatory 1000/class，遠在 C7 穩定區）。
...
- **v2 selector 候選形式〔作者終簽〕**：如 `argmax recall s.t. precision ≥ τ`，或 (coverage, recall) 非單調
  組合；具體形式與門檻隨 D 包登記、CIFAR-100 驗。
...
- **種子公式無碰撞〔作者終簽〕**：CIFAR-10 之 `seed*1e7+int(w*1e3)*1e4` 於本網格退化碰撞（`records/2026-07-06-05`
  §1.12）。CIFAR-100 提案 hash 派生（如 `int(hashlib.sha256(f"{seed}_{w}".encode()).hexdigest()[:8],16)`），
  附全網格枚舉唯一性之程式驗證輸出；公式與驗證一併登記。〔實作與驗證於終簽後、D commit 前落〕
- **base-model gate 定法〔作者終簽數字〕**：clean-fid ≤〔數字〕@〔張數〕；訓練已完成，gate 數字屬 D 範圍。
...
- **分支四版〔作者終簽一句話裁定是否另立第三份〕**。

## Follow-up

- **STOP：作者終簽**——D3 剪枝條件數、D4 功效配置、D5 probe 尺寸、D8 v2 形式、D9 種子公式與 base gate 數字、
  D12 分支四去留。一次過簽（D4/D8 已裁、本輪為終簽）。
- 終簽後（`records/2026-07-09-04` §3.4）：main 併回（`records/2026-07-07-01` 掛帳 8）→ D commit（早於任何
  CIFAR-100 取樣，git 可驗）→ E2 之 ⟨D⟩ 編號回填（-09-13）→ D10 時序啟動。
